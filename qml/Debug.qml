...
Window {
    ...
    // Cat sprite bottom edge sits at taskbar top
    property int catBottom: screenH - 48
    // Window bottom aligns to cat bottom
    ...
    // Timeout for chat responses stuck on "Thinking..."
    ...
    function connectivityMessage() {
        if (win.wsConnected) return "Hi! I'm Copilot Cat! Click me to chat!";
        return "Connecting to MCP server... 🐱";
    }

    function handleMcpMessage(data) {
        if (data.type === "show_bubble") {
            if (win.chatReplyReceived) return;
    ...
    function showBubble(text, isInput) {
        ...
        bubble.visible = true;
        // Delay resize so text layout computes bubble height first
        resizeDelay.start();
        if (isInput) {
            inputField.text = "";
            focusDelay.start();
    ...
    // Delay focus slightly so the window has time to accept input
    ...
    function resizeForBubble(expanded) {
        ...
        // Anchor: keep cat's right edge in place (cat peeks from right)
        var oldRight = x + width;

        width = targetWidth;
        height = targetHeight;
        x = clampX(expanded ? oldRight - width : oldRight - width);
                ...
                onClicked: function(mouse) {
                    ...
                    if (win.catState === "sit") {
                        if (win.wsConnected) {
                            ...
                            showBubble("Meow! Can't reach MCP server on ws://127.0.0.1:9922", false);
                        }
                    } else {
                        behaviorTimer.stop();
                        win.catState = "sit";
    ...
    SequentialAnimation {
        ...
        ScriptAction { script: showBubble(win.connectivityMessage(), false) }
        ...
        onTriggered: {
            ...
            if (win.catState === "walking") { win.catState = "idle"; interval = 2000 + Math.random() * 2000; }
                ...
                if (roll < 0.5) {
                    ...
                    // Playful jump: quick up-down bounce
                    win.catState = "jump";
                    jumpAnimation.start();
